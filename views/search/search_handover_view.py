...
def search_handover_view(page: ft.Page):
	# URLからhandover_idとキーワードを取得
	url_parts = urlparse(page.route)
	query_params = parse_qs(url_parts.query)
	
	# パスからIDを取得
	handover_id = int(page.route.split("/")[-1].split("?")[0])
	...
	return ft.Container(
		content=ft.Column(
			[
				...
				ft.Row(
					[
						ft.TextButton(text="戻る", icon="arrow_back", on_click=lambda e: page.go(f"/search_result?keyword={search_keyword}") if search_keyword else page.go("/")),
